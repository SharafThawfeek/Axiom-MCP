...
from axiom_debug.agent.client import get_client
from axiom_debug.agent.prompts import SYSTEM_PROMPT
from axiom_debug.agent.tools import TOOL_SCHEMAS, ToolExecutor
from axiom_debug.agent.verifier import filter_valid_citations, verify_semantic_consistency
from axiom_debug.config import settings
from axiom_debug.core.logger import logger
from axiom_debug.schemas.analysis import Analysis, AnalysisResponse, ParsedFailure
...
async def _create_completion(client, messages: list[dict]):
    ...
    for attempt in range(MAX_INTERACTIVE_RATE_LIMIT_RETRIES + 1):
        try:
            ...
            # Token accounting is best-effort telemetry, not part of the
            # result — getattr because a response without `usage` must not
